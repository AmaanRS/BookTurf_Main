...
class DatePicker extends HTMLElement {
  ...
  selectDay(el, day) {
    ...
    const utcDate = new Date(
      Date.UTC(this.date.year, this.date.monthNumber - 1, this.date.date)
    );
    ...
    this.toggleButton.setAttribute("value", isoDate);
...
`;
  ...
  getDateToggleValue() {
    const dateToggle = this.shadow.querySelector(".date-toggle");
    return dateToggle ? dateToggle.getAttribute("value") : null;
  }

  render() {
    const monthYear = `${this.calendar.month.name}, ${this.calendar.year}`;
    const date = this.date.format(this.format);
    const isoDate = this.date.Date.toISOString(); // Get the ISO string representation of the date
    this.shadow.innerHTML = `
      <style>${this.style}</style>
      <button type="button" class="date-toggle" name="date-toggle" id="date-toggle" value="${isoDate}">${date}</button>
